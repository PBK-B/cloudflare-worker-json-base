...
describe('deploy-cli', () => {
  ...
  it('resolves worker origin from wrangler deploy output', () => {
    expect(testing.resolveWorkerOrigin({ name: 'worker-json-base' } as any, 'Uploaded worker-json-base\n  https://worker-json-base.zmide.workers.dev')).toBe('https://worker-json-base.zmide.workers.dev')
    expect(testing.resolveWorkerOrigin({ name: 'worker-json-base' } as any, '')).toBe('https://worker-json-base.workers.dev')
  })
  ...
  it('rewrites config paths relative to generated config directory', () => {
    const rewritten = testing.rewriteConfigPathsForGeneratedFile({
      ...
      assets: { directory: 'dist/web/dash' },
      build: { watch_dir: 'apps/api/src' },
      d1_databases: [{ binding: 'JSONBASE_DB', migrations_dir: 'apps/api/database' }]
    } as any, path.join(process.cwd(), '.wrangler', 'deploy', 'generated', 'development'))

    expect(rewritten.main).toBe('../../../../apps/api/dist/index.js')
    expect((rewritten.assets as any).directory).toBe('../../../../dist/web/dash')
    expect((rewritten.build as any).watch_dir).toBe('../../../../apps/api/src')
    expect((rewritten.d1_databases as any[])[0].migrations_dir).toBe('../../../../apps/api/database')
  })
  ...
  it('deploy fails when wrangler.toml is missing', async () => {
    const wranglerPath = path.join(process.cwd(), 'apps', 'api', 'wrangler.toml')
    const backupPath = path.join(process.cwd(), 'apps', 'api', 'wrangler.toml.bak-test')
  ...
  it('deploy skips missing main path validation when build is explicitly skipped', async () => {
    ...
    try {
      await expect(testing.deploy({ nonInteractive: true, env: 'development', storage: 'd1', d1: 'jsonbase', confFile: confPath, skipMigrate: true, skipSecret: true, skipBuild: true, skipHealthcheck: true })).resolves.toBeUndefined()
    } finally {
      fs.unlinkSync(confPath)
    }
  })
