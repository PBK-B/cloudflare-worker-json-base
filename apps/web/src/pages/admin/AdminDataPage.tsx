...
import { useResourceApi } from '../../features/resources/api';
...
const AdminDataPage: React.FC = () => {
	...
	const { listData, deleteData, getResourceUrl } = useResourceApi();
	...
	const loadData = useCallback(async () => {
		...
		try {
			...
			if ((err as Error).name !== 'AbortError') {
				const message = err instanceof Error ? err.message : t('api.listFailed', { defaultValue: '获取列表失败' });
	...
	}, [listData, pagination.page, pagination.limit, searchQuery, sortBy, sortOrder, t]);
	...
	const renderSortHeader = (field: string, label: string) => {
		...
		return (
			<button type="button" className={`${styles.tableSortTrigger} ${isActive ? styles.tableSortActive : ''}`} onClick={() => handleSort(field)}>
				<span>{label}</span>
	...
	const handlePathContextMenu = useCallback((event: React.MouseEvent<HTMLButtonElement>, rowData: StorageData) => {
		...
		setContextMenu({ data: rowData, x: event.clientX, y: event.clientY });
	}, []);
	...
	return (
		<div className={styles.adminDataPage}>
			...
			<div className={styles.tableContainer}>
				...
				<Table height={400} data={dataList} loading={loading} autoHeight wordWrap="break-all" rowKey="id">
					...
					<Column flexGrow={1} fullText>
						...
						<Cell>
							{(rowData: StorageData) => (
								<button type="button" className={styles.pathCellButton} onContextMenu={(event) => handlePathContextMenu(event, rowData)} title={t('data.table.pathContextHint', { defaultValue: '右键打开菜单' })}>
									{rowData.id}
								</button>
							)}
						</Cell>
					...
					<Column width={100}>
						...
						<Cell>{(rowData) => <span className={getTypeBadgeClassName(rowData.type)}>{(rowData.type || 'UNKNOWN').toUpperCase()}</span>}</Cell>
					</Column>
			...
			{contextMenu ? (
				<div ref={menuRef} className={styles.contextMenu} style={{ left: contextMenu.adjustedX ?? contextMenu.x, top: contextMenu.adjustedY ?? contextMenu.y }} role="menu">
					<button type="button" className={styles.contextMenuAction} onClick={() => handleOpenInNewTab(contextMenu.data)}>
			...
			<div className={styles.pagination}>
				...
				<Pagination total={pagination.total} limit={pagination.limit} activePage={pagination.page} onChangePage={(page) => setPagination((prev) => ({ ...prev, page }))} />
			</div>
			...
			<Modal open={deleteModalOpen} onClose={() => setDeleteModalOpen(false)}>
				...
				<Modal.Footer>
					<Button onClick={() => setDeleteModalOpen(false)} appearance="subtle">{t('data.delete.cancel', { defaultValue: '取消' })}</Button>
					<Button onClick={confirmDelete} appearance="primary" color="red" loading={loading}>{t('data.delete.submit', { defaultValue: '删除' })}</Button>
				</Modal.Footer>
			</Modal>
			...
			<Modal open={errorModalOpen} onClose={() => setErrorModalOpen(false)}>
				...
				<Modal.Footer>
					<Button onClick={() => setErrorModalOpen(false)} appearance="primary">{t('data.delete.confirm', { defaultValue: '确定' })}</Button>
				</Modal.Footer>
			</Modal>
		</div>
	);
