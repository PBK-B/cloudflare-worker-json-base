...
import { usePermissionApi } from '../../features/permissions/api';
	...
	const loadRules = useCallback(async () => {
		...
		try {
			...
			if (response.success && response.data) {
				...
				const message = response.error || t('permissions.messages.loadFailed', { defaultValue: '获取权限规则失败' });
				setLoadError(message);
				setRules([]);
				notify.error(message);
			}
		} catch (error) {
			const message = error instanceof Error ? error.message : t('permissions.messages.loadFailed', { defaultValue: '获取权限规则失败' });
			setLoadError(message);
			setRules([]);
			notify.error(message);
		} finally {
		...
		<div className={styles.adminPermissionsPage}>
						...
						<Cell>
							{(rowData: PermissionRule) => (
								<span className={rowData.enabled ? styles.statusEnabled : styles.statusDisabled}>
									{rowData.enabled ? t('permissions.status.enabled', { defaultValue: '已启用' }) : t('permissions.status.disabled', { defaultValue: '已禁用' })}
								</span>
							)}
						</Cell>
					</Column>
						...
						<Cell>
							{(rowData: PermissionRule) => (
								<div className={styles.actionRow}>
									<Button size="sm" appearance="link" className={`${rowData.enabled ? styles.disableAction : styles.enableAction} ${styles.primaryAction}`} onClick={() => { void handleToggleStatus(rowData, !rowData.enabled); }}>
										{rowData.enabled ? <EyeOff size={14} /> : <Eye size={14} />}
										{rowData.enabled ? t('permissions.operations.disable', { defaultValue: '禁用' }) : t('permissions.operations.enable', { defaultValue: '启用' })}
									</Button>
			...
			<section className={styles.testerPanel}>
				...
				<div className={styles.testerControls}>
					...
					<SelectPicker data={actionOptions} cleanable={false} searchable={false} value={testerAction} onChange={(value) => setTesterAction((value as PermissionAction) || 'read')} />
					<Button appearance="primary" onClick={() => void runEvaluation()} loading={evaluationLoading}>
						{t('permissions.tester.run', { defaultValue: '开始测试' })}
					</Button>
				</div>
				<div className={`${styles.evaluationCard} ${evaluation ? (evaluation.allowed ? styles.evaluationAllowed : styles.evaluationPrivate) : styles.evaluationIdle}`}>
					{evaluation ? (
						<>
							<div className={styles.evaluationHeadline}>
								<span>{evaluation.allowed ? t('permissions.tester.publicAccess', { defaultValue: '当前操作为公开访问' }) : t('permissions.tester.privateAccess', { defaultValue: '当前操作需要私有访问' })}</span>
								<strong>{renderModeLabel(evaluation.mode)}</strong>
							</div>
							<p>{t('permissions.tester.matchedRule', { defaultValue: '命中规则：' })} {evaluation.matchedRule ? evaluation.matchedRule.pattern : t('permissions.tester.defaultRule', { defaultValue: '未命中规则，使用默认 private_rw' })}</p>
						</>
					) : (
			...
			<Modal open={modalOpen} onClose={() => setModalOpen(false)} size="md">
				<Modal.Header>
					<Modal.Title>{editingRule ? t('permissions.editTitle', { defaultValue: '编辑权限规则' }) : t('permissions.createTitle', { defaultValue: '新增权限规则' })}</Modal.Title>
				</Modal.Header>
				<Modal.Body>
					<Form fluid formValue={formValue as unknown as Record<string, unknown>} onChange={(value) => setFormValue(value as PermissionRuleInput)}>
						<Form.Group controlId="pattern">
							...
							<div className={styles.formHint}>{t('permissions.form.patternHint', { defaultValue: '支持 /foo/*、/foo/**、/**/*.png 这类路径匹配规则。' })}</div>
						</Form.Group>
						<div className={styles.formGridRow}>
							...
							<Form.Group controlId="priority" className={styles.formGridField}>
								...
								<div className={styles.formHint}>{t('permissions.form.priorityHint', { defaultValue: '数值越大优先级越高，多条规则命中时先应用高优先级规则。' })}</div>
							</Form.Group>
							<Form.Group controlId="enabled" className={`${styles.formGridField} ${styles.formGridFieldEnabled}`}>
								...
								<div className={styles.toggleFieldWrap}>
									<Toggle checked={formValue.enabled !== false} onChange={(checked) => setFormValue((prev) => ({ ...prev, enabled: checked }))} />
								</div>
							</Form.Group>
						</div>
						<Form.Group controlId="description">
							<Form.ControlLabel>{t('permissions.form.description', { defaultValue: '说明' })}</Form.ControlLabel>
							<Input as="textarea" rows={4} value={formValue.description || ''} onChange={(value) => setFormValue((prev) => ({ ...prev, description: value }))} />
						</Form.Group>
					</Form>
				</Modal.Body>
				<Modal.Footer>
					<Button onClick={() => setModalOpen(false)} appearance="subtle">{t('permissions.cancel', { defaultValue: '取消' })}</Button>
					<Button onClick={() => void handleSubmit()} appearance="primary" loading={submitLoading}>{t('permissions.save', { defaultValue: '保存' })}</Button>
				</Modal.Footer>
			</Modal>
			...
			<Modal open={Boolean(deleteTarget)} onClose={() => setDeleteTarget(null)} size="xs">
				...
				<Modal.Footer>
					<Button onClick={() => setDeleteTarget(null)} appearance="subtle">{t('permissions.cancel', { defaultValue: '取消' })}</Button>
					<Button onClick={() => void handleDelete()} appearance="primary" color="red">{t('permissions.delete', { defaultValue: '删除' })}</Button>
				</Modal.Footer>
			</Modal>
		</div>
	);
