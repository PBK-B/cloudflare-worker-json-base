...
const AdminLayout: React.FC = () => {
	...
	const { createData, uploadFile, replaceFile, updateData } = useResourceApi();
			...
			<Header className={styles.adminHeader}>
				<div className={styles.adminHeaderContent}>
					...
					<div className={`${styles.adminNav} ${styles.adminNavDesktop}`}>
						<NavLink to={appRoutes.admin} end className={({ isActive }) => `${styles.adminNavLink} ${isActive ? styles.navLinkActive : ''}`}>
							<Settings size={16} />
							{t('layout.navConsole', { defaultValue: '控制台' })}
						</NavLink>
						<NavLink to={appRoutes.adminData} className={({ isActive }) => `${styles.adminNavLink} ${isActive ? styles.navLinkActive : ''}`}>
							<Database size={16} />
							{t('layout.navData', { defaultValue: '数据管理' })}
						</NavLink>
						<NavLink to={appRoutes.adminPermissions} className={({ isActive }) => `${styles.adminNavLink} ${isActive ? styles.navLinkActive : ''}`}>
							<ShieldCheck size={16} />
